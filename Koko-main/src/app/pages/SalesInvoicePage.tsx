...
import type { SalesInvoice, SalesInvoiceItem, PaymentType } from '../types/accounting';
...
export function SalesInvoicePage() {
  ...
  const addItem = () => {
    setInvoiceItems([...invoiceItems, { item_id: '', quantity: 1, unit_price: 0, discount: 0 }]);
  };
  ...
  const updateItem = (idx: number, field: string, value: any) => {
    ...
    if (field === 'item_id') {
      ...
      if (item) {
        updated[idx].unit_price = item.selling_price;
      }
    }
  ...
  const handleSubmit = () => {
    const invoiceNumber = nextInvoiceNumber('sales');
    const invoiceId = Math.random().toString(36).substr(2, 9);
    const remaining = invoiceData.payment_type === 'cash' ? 0
      : invoiceData.payment_type === 'credit' ? total
      : total - invoiceData.paid_amount;
    const paid = invoiceData.payment_type === 'cash' ? total
      : invoiceData.payment_type === 'credit' ? 0
    ...
    const items: SalesInvoiceItem[] = invoiceItems
      .filter(i => i.item_id)
      .map(item => {
        const dbItem = state.items.find(i => i.id === item.item_id);
        const stock = state.itemStock.find(s => s.item_id === item.item_id && s.warehouse_id === invoiceData.warehouse_id);
        return {
          ...
          cost_price: stock?.average_cost || dbItem?.purchase_price || 0,
          warehouse_id: invoiceData.warehouse_id,
        };
      });

    dispatch({ type: 'CREATE_SALES_INVOICE', payload: { invoice, items } });
    setShowDialog(false);
    setInvoiceItems([]);
  ...
  const viewInvoice = state.salesInvoices.find(i => i.id === viewInvoiceId);
  const viewItems = state.salesInvoiceItems.filter(i => i.invoice_id === viewInvoiceId);
  const viewEntry = state.journalEntries.find(e => e.source_type === 'sales_invoice' && e.source_id === viewInvoiceId);
  const viewEntryLines = viewEntry ? state.journalEntryLines.filter(l => l.journal_entry_id === viewEntry.id) : [];

  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <h1 className="text-[20px] text-foreground">فواتير البيع</h1>
        <Button onClick={() => { setShowDialog(true); addItem(); }} className="gap-2 bg-emerald-600 hover:bg-emerald-700">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <Card><CardContent className="p-4 text-center">
          ...
          <p className="text-[20px]">{state.salesInvoices.length}</p>
        </CardContent></Card>
        <Card><CardContent className="p-4 text-center">
          <p className="text-[12px] text-muted-foreground">إجمالي المبيعات</p>
          <p className="text-[20px] text-emerald-600">{formatCurrency(state.salesInvoices.reduce((s, i) => s + i.total_amount, 0))} ج</p>
        </CardContent></Card>
        <Card><CardContent className="p-4 text-center">
          <p className="text-[12px] text-muted-foreground">المحصّل</p>
          <p className="text-[20px] text-blue-600">{formatCurrency(state.salesInvoices.reduce((s, i) => s + i.paid_amount, 0))} ج</p>
        </CardContent></Card>
        <Card><CardContent className="p-4 text-center">
          <p className="text-[12px] text-muted-foreground">المتبقي</p>
          <p className="text-[20px] text-orange-600">{formatCurrency(state.salesInvoices.reduce((s, i) => s + i.remaining_amount, 0))} ج</p>
        </CardContent></Card>
      </div>
          ...
          <Table>
            ...
            <TableBody>
              {state.salesInvoices.length === 0 ? (
                <TableRow><TableCell colSpan={8} className="text-center text-muted-foreground py-8">لا توجد فواتير بيع</TableCell></TableRow>
              ) : [...state.salesInvoices].reverse().map(inv => (
                <TableRow key={inv.id}>
      ...
      <Dialog open={showDialog} onOpenChange={v => { setShowDialog(v); if (!v) setInvoiceItems([]); }}>
        <DialogContent className="sm:max-w-3xl max-h-[90vh] overflow-y-auto" dir="rtl">
          <DialogHeader><DialogTitle>فاتورة بيع جديدة - {nextInvoiceNumber('sales')}</DialogTitle><DialogDescription className="sr-only">نموذج إنشاء فاتورة بيع جديدة</DialogDescription></DialogHeader>
          <div className="space-y-4">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <div>
                <label className="text-[13px] text-muted-foreground mb-1 block">العميل *</label>
              ...
              <div className="space-y-2">
                {invoiceItems.map((item, idx) => (
                  <div key={idx} className="grid grid-cols-12 gap-2 items-end bg-accent/30 p-2 rounded-lg">
                    ...
                    <div className="col-span-2">
                      {idx === 0 && <label className="text-[11px] text-muted-foreground block mb-1">الخصم</label>}
                      <Input type="number" value={item.discount} onChange={e => updateItem(idx, 'discount', Number(e.target.value))} className="h-8 text-[12px]" />
                    </div>
                    <div className="col-span-1 text-center">
                      {idx === 0 && <label className="text-[11px] text-muted-foreground block mb-1">الإجمالي</label>}
                      <span className="text-[12px]">{formatCurrency(item.quantity * item.unit_price - item.discount)}</span>
                    </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <div>
                <label className="text-[13px] text-muted-foreground mb-1 block">خصم إجمالي</label>
              ...
              <div>
                <label className="text-[13px] text-muted-foreground mb-1 block">ضريبة</label>
                <Input type="number" value={invoiceData.tax_amount} onChange={e => setInvoiceData({ ...invoiceData, tax_amount: Number(e.target.value) })} />
              </div>
              ...
              <div className="flex items-end">
                <div className="bg-emerald-50 p-3 rounded-lg w-full text-center">
                  <p className="text-[11px] text-emerald-600">صافي الفاتورة</p>
                  <p className="text-[18px] text-emerald-700">{formatCurrency(total)} ج</p>
                </div>
              </div>
            </div>
          </div>
          <DialogFooter>
            ...
            <Button onClick={handleSubmit} className="bg-emerald-600 hover:bg-emerald-700"
              disabled={!invoiceData.contact_id || invoiceItems.filter(i => i.item_id).length === 0}>
              حفظ الفاتورة
            </Button>
          </DialogFooter>
        </DialogContent>
      ...
      <Dialog open={showViewDialog} onOpenChange={setShowViewDialog}>
        <DialogContent className="sm:max-w-2xl max-h-[90vh] overflow-y-auto" dir="rtl">
          <DialogHeader><DialogTitle>فاتورة بيع - {viewInvoice?.invoice_number}</DialogTitle><DialogDescription className="sr-only">تفاصيل فاتورة البيع</DialogDescription></DialogHeader>
          {viewInvoice && (
            <div className="space-y-4">
              <div className="grid grid-cols-3 gap-4 text-[13px]">
                <div><span className="text-muted-foreground">العميل: </span>{getContactById(viewInvoice.contact_id)?.name}</div>
                ...
                <TableBody>
                  {viewItems.map(item => (
                    <TableRow key={item.id}>
                      ...
                      <TableCell className="text-[13px]">{formatCurrency(item.discount)}</TableCell>
              ...
              <div className="bg-accent/50 p-3 rounded-lg space-y-1 text-[13px]">
                <div className="flex justify-between"><span>الإجمالي:</span><span>{formatCurrency(viewInvoice.subtotal)} ج</span></div>
                {viewInvoice.discount_amount > 0 && <div className="flex justify-between"><span>الخصم:</span><span className="text-red-600">-{formatCurrency(viewInvoice.discount_amount)} ج</span></div>}
                {viewInvoice.tax_amount > 0 && <div className="flex justify-between"><span>الضريبة:</span><span>+{formatCurrency(viewInvoice.tax_amount)} ج</span></div>}
                <div className="flex justify-between border-t pt-1"><span>الصافي:</span><span className="text-[16px] text-emerald-700">{formatCurrency(viewInvoice.total_amount)} ج</span></div>
              </div>

              {/* Auto Journal Entry */}
              {viewEntryLines.length > 0 && (
                <div>
                  <h4 className="text-[14px] mb-2 flex items-center gap-2">
